# **************************************************************************
# * Kynex Sovereign - Retro-Go Master Dockerfile v212.0
...
RUN . /opt/esp/idf/export.sh && \
    ...
    ccache -C && \
    # 1. AŞAMA: Ana derleme (Cores hatasını yoksayıyoruz)
    (python3 rg_tool.py --target=esp32-s3-devkit release || true) && \
    # 2. AŞAMA: Bootloader ve Partition Table'ı manuel tetikliyoruz
    cd /app/launcher && \
    ...
    # 3. AŞAMA: Çıktıları kasaya (kynex_out) kilitle
    mkdir -p /kynex_out && \
